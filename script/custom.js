$(function(){
  ...
  $(window).resize(function(){
    ...
    if($(window).innerWidth() > 767) {
      $('.trigger').off('click.megaPc').on('click.megaPc', function(){
        $(this).toggleClass('active');
        $('.mega-navi').stop().slideToggle(300);
      });
      $('section').off('click.megaPc').on('click.megaPc', function(){
        $('.mega-navi').stop().slideUp(300);
      });
    }
    /* 디바이스 너비가 767 이하 일 때(모바일) */
    else if($(window).innerWidth() < 767) {
      $('.trigger').off('click.megaMo').on('click.megaMo', function(){
        $('.mega-navi').animate({'left': 0}, 300);
      });
      $('section, .btn-mega-navi-close').off('click.megaMo').on('click.megaMo', function(){
  ...
  /* Early Bird Countdown - 순수 자바스크립트 (N일 N시간 N분 N초) */
  (function() {
    var el = document.getElementById('early-bird-countdown');
    if (!el) return;
    ...
    function update() {
      ...
      if (diff <= 0) {
        el.innerHTML = '<b>이벤트가 종료되었습니다</b>';
        if (window._earlyBirdTimer) clearInterval(window._earlyBirdTimer);
        return;
      }
      ...
      el.innerHTML = '<b>종료까지</b> <em>' + days + '</em>일 <em>' + pad(hours) + '</em>시간 <em>' + pad(minutes) + '</em>분 <em>' + pad(seconds) + '</em>초';
    }
    update();
    window._earlyBirdTimer = setInterval(update, 1000);
  })();
  ...
  /* Category Main : 할인 필터 (레이아웃 유지) */
  if ($('.container.category-main').length) {
    var $categoryItems = $('.category-class-wrap .focus-class-item');
    $('.btn-earlybird').click(function () {
      $categoryItems.each(function () {
        var $el = $(this);
        var hasDiscount = $el.find('.sale, .badge.discount').length > 0;
        if (hasDiscount) {
          $el.show();
        } else {
          $el.hide();
